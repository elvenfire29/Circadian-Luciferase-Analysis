...
#Prism would want the same-treatment names in the same column
Treatments3<-PrismPeriod[1,]
Treatments000<-unique(Treatments3)
#only move data if treatments are repeated / multiple same-name treatments exist
if(length(Treatments3)!=length(Treatments000)){
 #redefine variables for easier manipulation with same variable name endpoint
  Pper<-PrismPeriod[-1,]
  Ppha<-PrismPhase[-1,]
  Pamp<-PrismAmplitude[-1,]
  ...
  blacklist3<-c()
  for(i in 1:length(Treatments3)){
    #find number of replicates per treatment
    Repeats<-which(Treatments3==Treatments3[i])
    #get all reps for a single treatment
    #for first treatment, define variables
    if(i==1){
      count=0
      for(k in Repeats){
        if(count==0){
          potkettle3x<-c(Pper[,k])
          potkettle4x<-c(Ppha[,k])
          potkettle5x<-c(Pamp[,k])
          count=count+1
        }
        #loop for all repeat col
        else{
          potkettle3x<-c(potkettle3x,Pper[,k])
          potkettle4x<-c(potkettle4x,Ppha[,k])
          potkettle5x<-c(potkettle5x,Pamp[,k])
        }
      }
    }
    #for all other treatments, define new variables
    else{
      #make sure we aren't repeating treatment replicates
      if((Treatments3[i] %in% blacklist3)==FALSE){
        count=0
        for(k in Repeats){
          if(count==0){
            potkettle3<-c(Pper[,k])
          ...
          #loop for all repeat col
            ...
            potkettle3<-c(potkettle3,Pper[,k])
            potkettle4<-c(potkettle4,Ppha[,k])
            potkettle5<-c(potkettle5,Pamp[,k])
          }
        }
      }
    }
    #make columns for new treatments by combining data with previous treatment columns
    if((Treatments3[i] %in% blacklist3)==FALSE){
      if(i!=1){
    if(nrow(as.data.frame(potkettle3x))!=nrow(as.data.frame(potkettle3))){
      if(nrow(as.data.frame(potkettle3x))>nrow(as.data.frame(potkettle3))){
        diff1<-nrow(as.data.frame(potkettle3x))-nrow(as.data.frame(potkettle3))
        for(u in 1:diff1){
          potkettle3<-c(potkettle3,"NA")
          potkettle4<-c(potkettle4,"NA")
          potkettle5<-c(potkettle5,"NA")
        }
      }
      if(nrow(as.data.frame(potkettle3x))<nrow(as.data.frame(potkettle3))){
        diff2<-nrow(as.data.frame(potkettle3))-nrow(as.data.frame(potkettle3x))
        for(u in 1:diff2){
          potkettle3x<-rbind(potkettle3x,"NA")
          potkettle4x<-rbind(potkettle4x,"NA")
          potkettle5x<-rbind(potkettle5x,"NA")
        }
      }
    }
    if(nrow(as.data.frame(potkettle3x))==nrow(as.data.frame(potkettle3))){
      potkettle3x<-cbind(potkettle3x,potkettle3)
      potkettle4x<-cbind(potkettle4x,potkettle4)
      potkettle5x<-cbind(potkettle5x,potkettle5)
    }
      }
    }
    blacklist3<-c(blacklist3,Treatments3[i])
  }
  PrismPeriod<-rbind(Treatments000,potkettle3x)
  PrismPhase<-rbind(Treatments000,potkettle4x)
  PrismAmplitude<-rbind(Treatments000,potkettle5x)
}
